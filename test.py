...
from models.utils_test import (calculate_error, solve_icp, point2inch,
...
parser.add_argument(
    '--resume_model', type=str, default='/home/chenghao/Mount/sch_ws/gnn/checkpoint/kitti/dgnn9-k[128, None, 128, None, 64, None, 64, None]-gap_loss3-FPFH/nomutualcheck-dgnn-k[128, None, 128, None, 64, None, 64, None]-batch64-distance-gap_loss3-FPFH-USIP/best_model_epoch_161(test_loss0.711704420218316).pth',
...
#     help='mine triplet_loss superglue')

# parser.add_argument(
#     '--net', type=str, default='raw', 
#     help='dgnn; raw')
...
parser.add_argument(
    # '--k', type=int, default=[64, 64], 
    # '--k', type=int, default=[128, 128, 128, 128, 64, 64, 64, 64], 
                ...
                for b in range(len(pred['idx0'])):
                    ...
                    if len(mkpts0) < 4:
                        ...
                        if opt.calculate_pose:
                            ...
                            if trans_error>2 or rot_error>5 and not np.isnan(trans_error) and not np.isnan(rot_error):
                                fail+=1
                                print('fail')
                            ...
                            if not np.isinf(rot_error_percent_vm) and not np.isinf(trans_error_percent_vm) and not np.isnan(trans_error_percent_vm) and not np.isnan(rot_error_percent_vm) and not np.isnan(relative_rot_error) and not np.isnan(relative_trans_error):
                    ...
                    ## 显示真值和匹配结果 ##
                    if opt.visualize:
                        plot_match(pc0, pc1, kpts0, kpts1, mkpts0, mkpts1, mkpts0_gt, mkpts1_gt, matches, mconf, true_positive, false_positive, T)
