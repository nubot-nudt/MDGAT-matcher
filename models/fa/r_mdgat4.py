...
def Rot3DPositionEncoder(desc0, pos, lrf):
    '''encode polar coordinate to rotation'''
    ...
    # pos = torch.einsum('bnxy,bny->bnx', lrf.transpose(2,3), pos)
    ...
    phi = phi.view(batch_dim, num_points)
    alpha = alpha.view(batch_dim, num_points)

    rot = torch.cat([alpha.unsqueeze(2), phi.unsqueeze(2)], 2)
    rot = F.interpolate(rot, size=[4], mode='nearest') # interpolate the dimension after 2th
    rot = rot.repeat(1, 1, np.int(feature_dim/(2*2)))
    rot = rot.unsqueeze(2).repeat(1,1,num_heads,1).transpose(1,3)

    return torch.mul(desc0,torch.cos(rot))+torch.mul(desc,torch.sin(rot))
...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, num_heads: int, d_model: int):
        ...
        self.proj2 = nn.Conv1d(d_model, d_model, kernel_size=1)
    ...
    def forward(self, x, source, x_pos, src_pos, lrf, name, k):
        ...
        if name=='self':
            '''v1: position information augmented q,k'''
            query = Rot3DPositionEncoder(query, x_pos, lrf)
            key = Rot3DPositionEncoder(key, src_pos, lrf)
...
class r_MDGAT4(nn.Module):
    ...
    def forward(self, data):
        ...
        '''data['Rt_z']用于旋转lrf。因为lrf估计是旋转不变的，而本实验中lrf是预先计算的，因此需要进行旋转消除。'''
